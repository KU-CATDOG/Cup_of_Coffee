...
public class CoffeeRecipeTest : MonoBehaviour
{
    ...
    private void Update() {
        if (Input.GetKeyDown(KeyCode.Escape)) { // 초기화
            for (int i = 0; i < coffee.Length; i++) {
                coffee[i] = 0;
            }
            Debug.Log("초기화 되었습니다.");
        }
        else if (Input.GetKeyDown(KeyCode.S)) { // 샷 1개
            coffee[0]++;
            Debug.Log("샷 1 추가");
        }
        else if (Input.GetKeyDown(KeyCode.W)) { // 물 50ml
            coffee[1]++;
            Debug.Log("물 50ml 추가");
        }
        else if (Input.GetKeyDown(KeyCode.M)) { // 우유 50ml
            coffee[2]++;
            Debug.Log("우유 50ml 추가");
        }
        else if (Input.GetKeyDown(KeyCode.K)) { // 우유 50ml 스팀
            coffee[3]++;
            Debug.Log("우유 50ml 스팀 추가");
        }
        else if (Input.GetKeyDown(KeyCode.B)) { // 우유 거품
            coffee[4]++;
            Debug.Log("우유 거품 추가");
        }
        else if (Input.GetKeyDown(KeyCode.I)) { // 얼음
            coffee[5]++;
            Debug.Log("얼음 추가");
        }
        else if (Input.GetKeyDown(KeyCode.C)) { // 초코펌프 1스푼
            coffee[6]++;
            Debug.Log("초코펌프 1 추가");
        }
        else if (Input.GetKeyDown(KeyCode.H)) { // 휘핑크림
            coffee[7]++;
            Debug.Log("휘핑크림 추가");
        }
        else if (Input.GetKeyDown(KeyCode.P)) { // 초코시럽
            coffee[8]++;
            Debug.Log("초코시럽 추가");
        }
        else if (Input.GetKeyDown(KeyCode.Space)) { // 확인
